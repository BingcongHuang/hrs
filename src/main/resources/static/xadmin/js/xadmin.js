;!function (win) {
  ...
  Xadmin.prototype.father_reload = function() {
    ...
    Xadmin.prototype.father_table_reload = function(id) {
        parent.table.reload(id);
    };
	/**
	 * [get_data 获取所有项]
	 * @return {[type]} [description]
 ...
 * @param div
 */
function openRightModel(url,width,div){
    var wi = $(document.body).width() - width;
    var a = '<div class="layui-layer-move"></div>';
    var shade = '<div class="layui-layer-shade" id="layui-layer-shade" style="z-index: 19891068; background-color: rgb(0, 0, 0); opacity: 0.1;" onclick="removeRightModel(this)"></div>';
    var html = '<iframe src="'+url+'" frameborder="0" scrolling="yes" class="x-iframe layui-layer layui-layer-page layui-anim " id="layui-layer" style="z-index: 19891069; width: '+width+'px; top: 0px; left: '+$(document.body).width()+'px;\n' +
        '    bottom: 0;height:100%;\n' +
...
function removeRightModel(_this) {
    $('#layui-layer').animate({
...
function topage(url){
    xadmin.add_tab('',url);
    $(".x-iframe", parent.document).attr("src",url);
    $('.left-nav', parent.document).find('a').removeClass('active');
    $('.left-nav #nav .sub-menu ', parent.document).find('li[data-url="'+url+'"]').children('a').addClass('active');
}
