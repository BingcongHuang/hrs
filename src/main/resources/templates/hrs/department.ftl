            ...
            <div class="layui-card">
                <div class="layui-card-body">
                    <div style="margin-top: 10px">
                        <div class="sub-title">
                            <div class="sub-title-header">科室管理</div>
...
<script type="text/html" id="dptToolbar">
    <div class="layui-btn-container">
        <button class="layui-btn layui-btn-normal layui-btn-sm" lay-event="addFisrtDpt">新增</button>
        <button class="layui-btn layui-btn-normal layui-btn-sm" lay-event="modifyFisrtDpt">修改</button>
        <button class="layui-btn layui-btn-normal layui-btn-sm" lay-event="delFisrtDpt">删除</button>
    </div>
</script>
...
<script>
    ...
    layui.use(['table', 'form'], function () {
        ...
        var dptTab = laytable.render({
            elem: '#departmentInfo',
            url: '${mainDomain}/cloud/hrs/department/flist/find',
            ...
            cols: [[
                ...
                , {field: 'dptType', title: '科室级别',templet:function (d){
                    if (d.dptType == 1) {
                        return "重点"
                    } else {
                        return "非重点"
                ...
                ,{field: 'opt', title: '操作',templet:function(d){
                        var opts='';
                        // 跳转品类属性页面
                        opts += '<button class="layui-btn layui-btn-xs layui-btn-normal propertyIcon" data-id="'+d.id+'" data-name="'+d.dptCname+'">二级科室管理</button>';
                        return opts;
                    }
                }
            ]]
        ...
        form.on('submit(departmentForm)',function (data) {
            ...
            dptTab.reload({
                contentType:"application/json;charset=UTF-8",
                where:{"dptCname":data.field.dptCname,"status":data.field.status}
            });
            return false;
        });
            ...
            switch(obj.event){
                case 'addFisrtDpt':
                    ...
                    var index = layer.open({
                        type: 2,
                        title: "新增一级科室",
                        content: '${mainDomain}/cloud/hrs/firstDpt/save',
                        area: ['800px', '500px'],
                        maxmin: true
                    });
                    break;
                case 'modifyFisrtDpt':
                    ...
                    var index = layer.open({
                        type: 2,
                        title: "修改一级科室",
                        content: '${mainDomain}/cloud/hrs/firstDpt/modify/'+data[0].id,
                        area: ['800px', '500px'],
                        maxmin: true
                    });
                    break;
                case 'delFisrtDpt':
                    ...
                    layer.confirm('是否确定删除?', function(index){
                        $.post("${mainDomain}/cloud/hrs/fisrtDpt/del/"+data[0].id,{},function(e){
                            layer.close(index);
                            if(e.code==0){
                                dptTab.reload();
                            }else{
    ...
    $(document).on("click",".propertyIcon",function(){
        location.href = mainDomain+'/cloud/hrs/secondDpt/page?id='+$(this).attr("data-id")+"&fName="+$(this).attr("data-name");
    });
